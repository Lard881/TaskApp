  ...
  void initState() {
    ...
    if (widget.initialFilter != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ref.read(taskFilterProvider.notifier).setFilter(widget.initialFilter!);
      });
    }
  }
  ...
  void _confirmDelete(Task task) {
    ConfirmationDialog.show(
      ...
      onConfirm: () async {
        ...
        if (context.mounted) AppSnackbar.show(context, AppStrings.taskDeleted);
      },
    );
  }
          ...
          children: [
            ...
            Expanded(
              child: tasksAsync.when(
                loading: () => const SkeletonTaskList(count: 6),
...
class _TaskRow extends ConsumerWidget {
  const _TaskRow({required this.task, required this.onTap, this.onLongPress});
  final Task task;
      ...
      child: Column(
        ...
        children: [
          ...
          ListTile(
            ...
            title: Text(
              task.isCompleted ? AppStrings.reopen : AppStrings.markComplete,
            ),
            onTap: () async {
              Navigator.of(context).pop();
              if (task.isCompleted) {
                await ref.read(tasksProvider.notifier).reopenTask(task.id);
                if (context.mounted) {
                  AppSnackbar.show(context, AppStrings.taskReopened);
                }
              } else {
                await ref.read(tasksProvider.notifier).markComplete(task.id);
                if (context.mounted) {
                  AppSnackbar.show(context, AppStrings.taskMarkedComplete);
                }
              }
            },
          ),
          ListTile(
            leading: const Icon(BootstrapIcons.trash, color: Colors.red),
            title: const Text(
              AppStrings.delete,
