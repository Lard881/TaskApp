...
class _NewConversationSheetState extends ConsumerState<NewConversationSheet> {
  ...
  List<WorkspaceMember> _filterContacts(List<WorkspaceMember> all) {
    final contacts = all.where((m) => m.userId != _currentUserId).toList();
            ...
            children: [
                  ...
                  child: ListView(
                    ...
                    children: _selectedIds.map((id) {
                      final member = allMembers
                          .where((m) => m.userId == id)
                          .firstOrNull;
                      final name = member?.profile?.firstName ?? id;
                      ...
                      return Padding(
                        ...
                        child: Chip(
                          ...
                          deleteIcon: const Icon(BootstrapIcons.x_lg, size: 14),
                          onDeleted: () =>
              ...
              Expanded(
                child: membersAsync.when(
                  loading: () =>
                      const SkeletonMemberList(count: 5),
                  error: (_, _) =>
                      const Center(child: Text('Could not load contacts.')),
                  data: (_) {
                    if (filtered.isEmpty) {
                      return Center(
                        child: Column(
