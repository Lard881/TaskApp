...
import 'package:planpal/application/providers/notification_provider.dart';
...
class TaskNotifier extends AsyncNotifier<List<Task>> {
  ...
  Future<void> addTask(Task task) async {
    ...
    if (task.dueDate != null) {
      ...
      if (prefs != null) {
        await ref.read(notificationServiceProvider).scheduleTaskReminder(task, prefs);
      }
    }
  ...
  Future<void> updateTask(Task task) async {
    ...
    if (task.dueDate != null && task.status != TaskStatus.completed) {
      ...
      final prefs = prefsAsync.valueOrNull;
      if (prefs != null) {
        await ref.read(notificationServiceProvider).scheduleTaskReminder(updated, prefs);
      }
    }
  ...
  Future<void> markComplete(String id) async {
    ...
    // Cancel future notifications and show completion notification
    await ref.read(notificationServiceProvider).cancelTaskReminder(task);
    final prefsAsync = ref.read(preferencesProvider);
    final prefs = prefsAsync.valueOrNull;
    if (prefs != null) {
      await ref.read(notificationServiceProvider).notifyTaskCompleted(task, prefs);
    }
...
final performanceProvider = Provider<PerformanceMetrics>((ref) {
  return ref.watch(tasksProvider).when(
    ...
    error: (_, _) => const PerformanceMetrics(),
  );
});
